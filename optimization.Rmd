...
This guide outlines some of the many tools and tips for improving the speed and 
execution of your R code. 

Sometimes, simply tweaking a few lines of code can lead to large performance 
gains in the execution of your program. Other issues may take more time to work 
through but can be a huge benefit to your project in the long term. An 
important lesson to learn when it comes to optimising your R (or any) program 
is knowing when to stop. Be sure to consider what "fast enough" is for your 
project and how much needs to be optimised.   
...
It is easy to let your version of R lapse and lag behind the release schedule, 
but this is one of the easiest ways to improve the performance of R! In general, 
...
Often times, time-intensive R code can be sped up by breaking the execution of 
the job across additional cores of your computer.

## Learn `lapply`/`purrr::map`

The first step in learning to run R code in parallel is learning the `lapply` 
(and variants) function from Base R or the `map` (and variants) function from 
the `purrr` package. Once you understand how `lapply` and `map` work, running 
your code in parallel will be simple.

Say you have a vector of numbers and want to find the square root of each one
(ignore for now that `sqrt` is vectorised, which will be covered later). 
...
Those working within the `tidyverse` may use `map` from the `purrr` package 
equivalently:
...
file.size('optimization/iris.csv') / 1000
df <- readr::read_csv('optimization/iris.csv')
pryr::object_size(df)
```
...
Each of these data types occupy a different amount of space in memory - 
...
```{r size-03}
x <- sample(letters, 10000, replace = T)
pryr::object_size(as.character(x))
...
avoided at all costs. This is not true! Rather, an improperly formatted loop 
...
(and not just a single value) as input. The `sqrt` function is one:
