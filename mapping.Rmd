...
(coming soon!). Now let's start mapping!
...
`r knitr::include_graphics(here::here("mapping", "www", "images", "geospatial_workflow.jpg"), dpi = 600)`
...
dc_parks = st_read("mapping/data/dc_parks.geojson", quiet = T)
...
```{r append-spatial-info, warning = F, message = F, cache = F}
library(urbnmapr)
...
```{r append-state-pops, message = F, include = F, eval = T, echo = F}
# TODO: DELETE THIS

# Read in data on state populations from 2010
state_pops <- 
	read_csv("mapping/data/state_populations.csv",
...
## What are Coordinate Reference Systems {.tabset .tabset-pills}
...
all datasets you are using have the sae CRS, or to use a projected CRS
...
one, see the `Get Spatial Data` section above.
...
```{r first-map, cache = FALSE, cache = TRUE}
library(urbnmapr)
...
chip_with_geographies %>% 
	...
	# adjust calors manually to be on Urban cyan palette
...
```{r bubble_maps, warning = F, message = F, cache = TRUE}
set_urbn_defaults(style = "map")
...
```{r dot_density_map, message = F, warning = F, cache = TRUE}
library(tidycensus)

# Get counts by race of DC tracts
dc_pop = get_acs(geography = "tract",
								 state = "DC",
								 year = 2019,
								 variables = c(
								 	Hispanic = "DP05_0071",
...
alternative to chorpleth maps, which tend to overemphasize
...
ggplot() + 
	geom_sf(data = chip_with_geographies_weighted, 
...
Cropping (or clipping) is geographically filtering an `sf` datframe to just 
the area we are interested in. Say we wanted to look at the roads around Fire 
Station 24 in DC. 

```{r roads_cropping_before, cache = T, warning = F, message = F}
library(tigris)
library(units)
...
ggplot() +
		...
		geom_sf(data = fire_station_24_buffered,
...
We can clip the larger roads data to just roads that overlap with the circle 
around the fire station with `st_intersection()`.

```{r roads_cropping_after}

# Use st_intersection() to to crop the roads data to just roads within the
# fire_station radius
...
ggplot() +
	...
	geom_sf(data = dc_roads_around_fire_station_24_buffered, 
					color = palette_urbn_gray[7]) +
	theme_urbn_map()
