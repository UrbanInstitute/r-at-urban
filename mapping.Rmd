...
[`Map Gallery`](#map_gallery). Now let's start mapping!
...
have an `sf` dataframe. In all the examples, we have already run
`set_urbn_defaults(style = "map")` to get Urban formatting.
...
# Helper function to get correct factor order for intervals
get_correct_level_order_for_intervals = function(x){
  text_levels = text %>% levels()
  
  factor_rank = text_levels %>% 
    str_split("%") %>% 
    map_chr(~.[[1]]) %>% 
    as.numeric() %>% 
    rank()
  
  return(text_levels[factor_rank])
}
...
format_interval <- function(interval_text) {
  ...
  text <- text %>%
    ...
    map(~ as.numeric(.x) %>%
      ...
      paste0(collapse = " — ")) %>%
    unlist() %>% 
    # Use custom func to order levels of the interavls appropriately for maps
    fct_relevel(get_correct_level_order_for_intervals(.)) %>% 
    fct_rev()
...
chip_with_geographies <- chip_with_geographies %>%
  mutate(chip_pct = rnorm(n = nrow(.),mean = 0.1, sd = 0.02)) %>% 
...
These maps scatter dots within a geographic area where dot represents a
unit (like 100 people, or 1000 houses). They allow for representation of
the internal heterogeneity of geographic units and are a great
alternative to choropleth maps.

To create this kind of map, you need to start with an `sf` dataframe
that is of `geometry` type `POLYGON` or `MULTIPOLYGON` and then use
`as_dot_density()` from `library(tidycensus)`.

```{r}
library(tidycensus)

# Get counts by race in each DC tract
dc_pop <- get_acs(
  geography = "tract",
  state = "DC",
  year = 2019,
...
dc_race_dots = dc_pop %>% 
  as_dot_density(
    ...
    value = "estimate",
    # How many values should each dot represent?
    values_per_dot = 100,
    group = "variable",
    erase_water = FALSE
  )
...
Probably the most user friendly of the interactive mapping R libraries.
All you have to do to create an interactive map is:

```{r show-mapview}
library(mapview)


any_sf_dataframe <- chip_with_geographies %>%
...
mapview(any_sf_dataframe, zcol = "chip_enrollment")
```
...
```{r mapview-layers}
mapview(any_sf_dataframe, col.regions = palette_urbn_green[5]) +
...
```{r mapview-sliders}
# you many need to install the below library
...
Cropping (or clipping) is geographically filtering dataset to just the
area we are interested in. Say we wanted to look at the roads around
Fire Station 24 in DC.

```{r roads_cropping_before, cache = TRUE, echo = FALSE}
library(tigris)
library(units)
...
Use `st_intersection` to clip the larger roads dataset to just roads
that overlap with the circle.

```{r roads_cropping_after}

# Use st_intersection() to crop the roads data to just roads within the
# fire_station radius
dc_roads_around_fire_station_24_buffered <- dc_roads %>%
  st_intersection(fire_station_24_buffered)
...
If you want to clip to data that intersects with the bounding box of the
circle, use `st_crop()`:

```{r}
dc_roads_around_fire_station_24_clipped <- dc_roads %>%
  st_crop(fire_station_24_buffered)
...
ggplot() +
  ...
  geom_sf(
    data = dc_roads_around_fire_station_24_clipped,
    color = palette_urbn_gray[7]
  ) +
  theme_urbn_map()
```
