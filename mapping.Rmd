...
This guide will be split into sections describe each of the steps.
...
There are many R packages that make this easy, depending on the kind of
spatial data you want to read in.
...
-   `crs`: The CRS (coordinate references system) for your
    ...
    information on finding and setting CRS codes, please see this
    section. (TODO: INSERT CRS LINK)
...
```{r readin-chip-data, warning=FAlSE, message=FALSE}
...
or the right hand side dataframe (in this case `chip`). We recommend
using `tidylog::left_join()` to confirm there were no unmatched records.
...
-   It's important that all spatial data sets you are working with be in
...
-   The Urban Institute Style Guide requires the use of the Atlas Equal
    Earth Projection (`"ESRI:102003"`) for US maps. For state and local
...
1)  [**Geographic coordinate
    ...
    -   Usually use decimal degree units (ie latitude/longitude) to
        ...
        `r knitr::include_graphics(here::here("mapping", "www", "images", "gsc_image.png"), dpi = 150)`
...
2)  [**Projected coordinate
    ...
    -   Specifies a 3d shape for the earth + a 2d mapping
        ...
        -   Is a geographic coordinate system + a *projection*
            ...
            credit:
            [xkcd](%5B%60https://imgs.xkcd.com/comics/projecting.png%60%5D(https://imgs.xkcd.com/comics/projecting.png))
...
You can also the `st_crs()` function to find the CRS associated with
your `sf` dataframe. Note that the information returned is very long,
but the key information is located at the end in `ID[authority, SRID]`.
For example:
...
Sometimes however, the CRS will be blank or `NA` as the dataset did not
specify the CRS. In that case you MUST find and set the CRS for your
data before proceeding with analysis. Below are some good rules of thumb
...
`crsuggest` R [package]() for help identifying the CRS in your data.
Once you've identified the appropriate CRS, you can set the CRS for your
data with `st_crs()`:

```{r set_crs, eval = F}
# TODO: Make this into a crsuggest example
...
Often you will need to change the CRS for your `sf` dataframe. You can
do this with `st_transform`:
...
if you are working with multiple `sf` dataframes which have different
CRS, you will need to transform the CRS to all be the same. Luckily,
`st_transform` allows you to just use the CRS of another `sf` dataframe
when transforming, so an easy way to do this would be
...
one, see the `Importing Spatial Data` section above.
...
The key function for mapping is the special `geom_sf()` geom which works
...
charts can be used for maps too. For instance, to change the colors of
the map above, just use the `fill =` and `color =` parameters in
`geom_sf()`. `fill` will change the fill color of polygons; `color` will
change the color of polygon outlines, lines, and points.
...
Viz Style guide. If for example you want states to be Urban's magenta
color:
...
set the `fill` argument equal to a variable in your data inside the
`aes()` argument of `geom_sf`. As an example:
...
When we set an aesthetic mapping inside `aes()`, we are telling R that
we want some aspect of the plot to change in accordance with a variable
in our dataset. And because we want the fill color to vary, we use the
`fill` aesthetic.

### Continuous color scale

Let's say we wanted to make a choropleth map of states colored in by
...
And if we wanted to turn the text in the legend to be percentages, we
can use the `labels` argument of the `scale_fill_continuous` function.
...
legend using the `scale_fill_steps()` function.
...
Be careful when doing this as changing the number or size of bins can
drastically change how the map looks. It might also be helpful to
manually create your bins yourself by converting your continuous
variables into factors. This will give you more fine grained control
over exactly what the text looks like in the legend. Below is an example
...
| Column     | Description                                          |
+:===========+:=====================================================+
| Ma         | The actual address that the inputted address was     |
| tch_addr   | matched to. This is the address that the geocoder    |
...
+------------+------------------------------------------------------+
| A ddr_type | The match level for a geocode request. This should   |
...
+------------+------------------------------------------------------+
| Re         | Abbreviated State Name. For example, CA for          |
| gionAbbr   | California                                           |
+------------+------------------------------------------------------+
