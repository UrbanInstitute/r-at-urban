...
These maps scatter dots within a geographic area where dot
represents a unit (like 100 people, or 1000 houses). They allow for
representation of the internal heterogeneity of geographic units and are a 
great alternative to choropleth maps.

To create this kind of map, you need to start with an `sf` dataframe that is of `geometry`
type `POLYGON` or `MULTIPOLYGON` and then use `as_dot_density()` from `library(tidycensus)`.
...
Probably the most user friendly of the interactive
...
Cropping (or clipping) is geographically filtering dataset to just 
...
Use `st_intersection` to clip the larger roads dataset to just roads that 
...
If you want to clip to data that intersects with the bounding box of the circle, 
use `st_crop()`:
