...
![](mapping/www/images/amsterdam.JPG)
...
`urbnmapr` provides sf objects for mapping US states and counties. The `get_urbn_map()` function allows you to load both states and counties, with options for including territories. For information on how to install urbnmapr, see the [GitHub repository](https://github.com/UrbanInstitute/urbnmapr).
...
`library(tigris)` allows you to easily download TIGER and cartographic boundaries from the US Census Bureaus. In order to load in the boundaries as `sf` objects, run `r options(tigris_class = "sf")` once per session. 
...
Unlike `urbnmapr`, different functions are used for different geographic levels- for instance, the `blocks()` function will load census block groups, and the `states()` function will load states. For the full list of functions, see the [package vignette](https://cran.r-project.org/web/packages/tigris/tigris.pdf).
...
list.files("mapping/shapefiles")

dc <- st_read(dsn = "mapping/shapefiles",
							layer = "Basemap_of_DC_in_Dark_Gray")
...
To install `urbnthemes`, visit the [GitHub repository](https://github.com/UrbanInstitute/urbnthemes).
...
For more information on creating Urban-styled maps, see the [Urban Institute Data Visualization Style Guide](http://urbaninstitute.github.io/graphics-styleguide/).
...
`library(urbnmapr)` is a tool for making stats and county choropleth maps of the United States, with Alaska and Hawaii shifted and scaled to be displayed next to the continental US. 

Variables mapped as colors or shades need to be joined using `left_join()` to geographic data loaded by `library(urbnmapr)`. In this example, the variable `"state_name"` is used to join the geographic data to the data we wish to display.  
...
ggplot() +
	geom_sf(data = state_data, mapping = aes(fill = horate),
					color = "white") +
...
# load shapefiles for the three states
state <- get_urbn_map(map = "states", sf = TRUE) %>%
	filter(state_name %in% c("Washington", "Oregon", "Idaho")) 

ggplot() +
	geom_sf(data = state, mapping = aes(),
					color = "#ffffff", fill = "gray", size = 0.25) +
	labs(title = "Northwest United States") +
	# use UTM zone 10, projected CRS used in the Pacific Northwest
	coord_sf(crs = 32610) +
	theme_urbn_map()
...
# load the shapefiles for counties, join with county level data
county <- get_urbn_map(map = "counties", sf = TRUE) %>%
	filter(state_name %in% c("Washington", "Oregon", "Idaho")) %>% 
	left_join(countydata, by = "county_fips")

# load the shapefiles for states
state <- get_urbn_map(map = "states", sf = TRUE) %>%
	filter(state_name %in% c("Washington", "Oregon", "Idaho")) 
...
ggplot() +
	...
	geom_sf(data = state,
					mapping = aes(),
					color = "#ffffff", fill = NA, size = 0.6) +
	scale_fill_gradientn(labels = scales::dollar) +
	# use UTM zone 10, projected CRS used in the Pacific Northwest
	coord_sf(crs = 32610) +
	theme_urbn_map() +
	labs(title = "Household Income by County in the Northwest United States",
			 fill = "Median household incone")
```
