...
<div id="header">
![](docs/mapping/images/urban-institute-logo.png)
</div>
...
![](docs/mapping/images/amsterdam.JPG)
...
chip <- read_csv("docs/mapping/data/chip-enrollment.csv")
...
hmda <- read_csv("docs/mapping/data/hmdamap2015_10_purch.csv", 
  col_types = cols(
    tract = col_character(),
    year1 = col_integer(),
...
tracts_dc <- readOGR(dsn = "docs/mapping/shapefiles/tl_2015_11_tract", verbose = FALSE)
...
state_capitals <- read_csv("docs/mapping/data/state-capitals.csv") %>%
...
mortgages <- read_csv("docs/mapping/data/districtofcolumbia2011.csv")
...
shapes <- readOGR(dsn = "docs/mapping/shapefiles/tl_2015_11_tract") %>%
    tidy()
...
get_coordinates <- function(address) {
  ...
  # geocode the address (returns a complicated data structure)
	geo_reply <- geocode(location = address, output = 'all', messaging = TRUE)
...
![](docs/mapping/images/shapefile.png)
...
readR <- function(shapefile) {
  ...
  # Output: Tibble with observations for every ten mortgages
	...
	tracts_shapefile <<- readOGR(paste0("docs/mapping/shapefiles/", shapefile))
...
purchase <- read_csv("docs/mapping/data/hmdamap2012_10_purch.csv", 
	col_types = cols(
  tract = col_double(),
  year1 = col_integer(),
  tract_T = col_character(),
