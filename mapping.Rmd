...
geographies like Census Tracts. To make a choropleth map, you need to set the
`fill` argument equal to a variable in your data inside the `aes()` argument of 
`geom_sf`. As an example:
...
When we set an aesthetic mapping inside `aes()`, we are telling R that we want some
...
And if we wanted to turn the text in the legend to be percentages, we can use 
the `labels` argument of the  `scale_fill_continuous` function.
```{r}
chip_with_geographies_map +
