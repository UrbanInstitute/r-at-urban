...
* The following maps are styled by the [Urban Institute ggplot2 theme](https://github.com/UrbanInstitute/urban_R_theme) which can be called on Windows with `source("https://raw.githubusercontent.com/UrbanInstitute/urban_R_theme/master/urban_theme.R")`. 
...
`library(urbnmapr)` is an R package created by the Urban Institute. It includes state and county outlines for the entire United States with Hawaii and Alaska next to the Continental U.S. The packages includes a data frame for states called `states` and a data frame for counties called `counties`.
...
# between the states
ggplot(data = chip, mapping = aes(long, lat, group = group, fill = enrollment_group)) +
...
```{r projection, include=FALSE, eval=FALSE}
library(USAboundaries)
...
head(richmond_dc) %>%
	kable(digits = 2, caption = "Output from ggmap::trek()", format = "html", align = "l") %>%
	kableExtra::kable_styling(full_width = FALSE, position = "left")
```

```{r map-trek, eval=FALSE}
# map trek data
...
head(richmond_dc) %>%
	kable(digits = 2, caption = "Output from ggmaps::route", align = "l", format = "html") %>%
	kableExtra::kable_styling(full_width = FALSE, position = "left")
```
