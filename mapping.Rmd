...
mapping in R is an `sf` dataframe. An `sf` dataframe is essentially a regular
...
without losing too much accuracy. To load cartographic boundaries,
include the `cb = TRUE` argument. If you are doing detailed
...
function will load tract data. For the full list of supported geographies and
...
used in a spatial dataset and optionally how we project that 3d shape
...
the same CRS in order for spatial opperations and maps to be accurate.
...
it. There are many ways to do it, but all will create [distortions](https://twitter.com/neilrkaye/status/1050740679008296967) of
...
1) [**Geographic coordinate systems**](https://www.ibm.com/support/knowledgecenter/en/SSGU8G_12.1.0/com.ibm.spatial.doc/ids_spat_407.html) (sometimes called unprojected coordinate systems)
...
```{r, eval = T, echo = F, out.width = 300, out.width=300}
	knitr::include_graphics(here("mapping", "www", "images", "gcs_esri.png"))
```
...
2) [**Projected coordinate systems**]()
    * Specifies a 3d shape for the earth + a 2d mappin
		...
		* All projections distort the true shape of the earth in some way, either in terms of shape, area, or angle. Required [xkcd comic]([https://xkcd.com/977/)](https://xkcd.com/977/))
...
Sometimes however, this CRS field will be blank or be `NA` as the dataste did not specify the CRS. In that case you must find and set the CRS for your data before proceeding with analysis. Below are some good rules of thumb for finding out what CRS your data is in:
...
If none of the above rules of thumb apply to you, check out the [Whattheproj](https://stevage.github.io/WhatTheProj/), or the `crsuggest` R [package]()

### Setting the CRS

The `sf` package has a number of handy functions for identifying,
setting, or transforming rthe CRS for a spatial dataset.
...
allow you to most effectively do that. For example, if you really want
