...
urbnmapr::get_urbn_map("states", sf = TRUE) %>% 
	head(5)
...
`urbnmapr` provides sf objects for mapping US states and counties. The `get_urbn_map()` function allows you to load both states and counties, with options for including territories.
...
`library(tigris)` allows you to easily download TIGER and cartographic boundaries from the US Census Bureaus. In order to read in the boundaries as `sf` objects, run `r options(tigris_class = "sf")` once per session. 
...
Once you have geospatial data, you have everything you ne
...
* The following maps are styled by the Urban Institute package [urbnthemes](https://github.com/UI-Research/urbnthemes). 
* U.S. dot maps and U.S. choropleths use the Albers Equal-Area Conic Projection.
* Maps that show the magnitude of a variable use a sequential ramp and maps that display positives and negatives use the diverging color ramp. 
...
Simple features are data frame that contain a column with geographic information. Special features work well with `tidyverse` data manipulation tools and `ggplot2` plotting. 

`library(sf)` includes tools for reading, writing, and manipulating spatial data. Many common tasks done in ArcGIS, QGIS, or other geospatial analysis software can be easily replicated.

#### Reading in data

There are several packages that can help load common shapefiles. The function `st_read()` can be used to read in your own shapefiles.
...
```{r google geocode single address, eval=FALSE}
# load the necessary packages
library(tidyverse)
library(ggmap)
...
get_coordinates(address = "1600 Monument Ave. Richmond, Virginia 23220") %>%
	kable(digits = 2, caption = "Output from geocoding one address", format = "html", align = "l") %>%
...
```{r google geocode multiple addresses, eval=FALSE}
...
tibble(institution = institutions,
	...
	select(-coords) %>%
	kable(digits = 2, caption = "Output from geocoding multiple addresses", format = "html", align = "l") %>%
	kableExtra::kable_styling(full_width = FALSE, position = "left")
```
...
data.frame(institution = institutions, 
	...
	kable(digits = 2, caption = "Output from ggmap::mutate_geocode", format = "html", align = "l") %>%
	kableExtra::kable_styling(full_width = FALSE, position = "left")
