...
6. Tract to tract driving times and distances (2017)
...
The documentation are self-explanatory. Census tracts are 2010 vintage, and the columns are the origin tract, destination travel, travel time in minutes, and travel distance in miles. These data were calculated by the Data Science team at Urban.
...
Open Street Map is an open source, crowdsourced dataset, which means it is constantly updated by a group of dedicated volunteers, and therefore may have some errors, be dated in certain locations, and contain omissions.

We use the routing engine to calculate the fastest driving distance and time with no traffic between each of the over 154 million pairs, using the OSRM /route/v1/driving endpoint. For more information on how these routes are calculated, see [OSRM](http://project-osrm.org/docs/v5.7.0/api/#general-options"). 

For the full process to create the data yourself, see the Github repository at [https://github.com/UI-Research/spark-osrm](https://github.com/UI-Research/spark-osrm).

### Data Available

All Census tract pairs within 150 miles of each other by straight line distance are included, using data from 2017 and 2010 Census tracts, per the documentation above.

### File Locations

The entire dataset is available as a single file, at: 

- s3n://ui-spark-data/travel-times/parquet-data/drive_times.parquet
