...
To analyze the ZTRAX data, first load the sparklyr and dplyr libraries, set up the Spark configuration, and provide your access keys.

```{r setup2, include=FALSE, eval = FALSE}
config[["fs.s3a.access.key"]] = "YOUR ACCESS KEY"
config[["fs.s3a.secret.key"]] = "YOUR SECRET KEY"
```
...
You may want to take a CSV extract that's too big to fit into memory, or takes too long to collect and process. Use the following function:
...
write_out <- function(dataset, namein, bucket_path){
  ...
  spark_write_csv(dataset,pathin,header = FALSE)
  ...
  system('aws s3 rm ${inputpath} --recursive')
